...
if __name__ == "__main__":
    ...
    dummy_raster = xr.DataArray(
        data=np.random.choice([0, 0.25, 0.5, 0.75, 1.], size=(resolution, resolution)),
        coords={"lat": np.linspace(11.5, 10, resolution), "lon": np.linspace(0, 1.5, resolution)},
        dims=["lat", "lon"],
    )
    ...
    segmentation_2 = gpd.GeoDataFrame(
            ...
            'geometry': get_square_segmentation((-0.25, 1.75), (9.75, 11.75), int(resolution/2))
            },
            crs="EPSG:4326"
        )
    ...
    segmentation_3 = gpd.GeoDataFrame(
            ...
            'geometry': get_square_segmentation((-0., 1.5), (10, 11.5), resolution-1)
            },
            crs="EPSG:4326"
        )
